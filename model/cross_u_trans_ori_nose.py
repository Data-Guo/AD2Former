...
class cross_u_trans(nn.Module):
    def __init__(self, n_channels=3, num_classes=9, heads=8, depth=3, dim=320):
        ...
        self.vit_1 = TransformerModel(dim=128, mlp_dim=2 * 128 , heads=heads, depth=depth)
        self.vit_2 = TransformerModel(dim=256, mlp_dim=2 * 256, heads=heads, depth=depth)
        self.vit_3 = TransformerModel(dim=512, mlp_dim=2 * 512, heads=heads, depth=depth)
